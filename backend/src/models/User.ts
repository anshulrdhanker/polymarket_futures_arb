...
export interface GmailTokens {
  ...
  scope?: string;        // Made optional to match Gmail API
  token_type?: string;   // Made optional to match Gmail API
  expiry_date?: number;  // Made optional to match Gmail API
}
...
const CAMPAIGN_LIMITS: Record<SubscriptionTier, number> = {
  free: 1,
  basic: 5,
  pro: 50,
  enterprise: 500
...
export class User {
  ...
  constructor(profile: UserProfile) {
    ...
    this.subscription_tier = profile.subscription_tier;
  ...
  /**
   * Check if user can create more campaigns based on their subscription
   */
  async checkCampaignLimits(): Promise<{ allowed: boolean; currentUsage: number; limit: number }> {
    const currentDate = new Date();
    const billingStart = new Date(this.billing_cycle_start);
    
    // Reset counter if it's a new billing cycle
    if (currentDate > billingStart) {
      await this.resetCampaignUsage();
    }

    const limit = CAMPAIGN_LIMITS[this.subscription_tier as SubscriptionTier] || 0;
    return {
      allowed: this.campaigns_used_this_month < limit,
      currentUsage: this.campaigns_used_this_month,
      limit
    };
  }
  ...
  getNextBillingDate(): Date {
    const nextBilling = new Date(this.billing_cycle_start);
    nextBilling.setMonth(nextBilling.getMonth() + 1); // Monthly billing cycle
    return nextBilling;
  }

  /**
   * Reset campaign usage counter
   */
  private async resetCampaignUsage(): Promise<void> {
    try {
      ...
      if (error) {
        console.error('Error resetting campaign usage:', error);
        throw error;
      }

      this.campaigns_used_this_month = 0;
      this.billing_cycle_start = new Date().toISOString();
    } catch (error) {
      console.error('Failed to reset campaign usage:', error);
      throw error;
    }
  }
  ...
  async incrementCampaignUsage(): Promise<void> {
    try {
      ...
      this.campaigns_used_this_month += 1;
    } catch (error) {
